...
func TestCopySameStructWithPointerField(t *testing.T) {
	...
	user := &User{Birthday: &currentTime, Name: "Zhangsan", Nickname: "zhangsan", Age: 18, FakeAge: &fakeAge, Role: "Admin", Notes: []string{"hello world", "welcome"}, flags: []byte{'x'}}
	newUser := &User{}
	copit.Copy(newUser, user)
...
func TestCopyStruct(t *testing.T) {
	...
	if err := copit.Copy(employee, &user); err == nil {
	...
	employee2 := Employee{}
	copit.Copy(&employee2, user)
	checkEmployee(employee2, user, t, "Copy From Struct To Ptr")

	employee3 := Employee{}
	ptrToUser := &user
	copit.Copy(&employee3, &ptrToUser)
	checkEmployee(employee3, user, t, "Copy From Double Ptr To Ptr")

	employee4 := &Employee{}
	copit.Copy(&employee4, user)
	checkEmployee(*employee4, user, t, "Copy From Ptr To Double Ptr")
}

func TestCopyFromStructToSlice(t *testing.T) {
	user := User{Name: "Zhangsan", Age: 18, Role: "Admin", Notes: []string{"hello world"}}
	employees := []Employee{}

	if err := copit.Copy(employees, &user); err != nil && len(employees) != 0 {
		fmt.Println(err)
		t.Errorf("Copy to unaddressable value should get error")
	}

	if copit.Copy(&employees, &user); len(employees) != 1 {
	...
	employees2 := &[]Employee{}
	if copit.Copy(&employees2, user); len(*employees2) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copit.Copy(&employees3, user); len(employees3) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copit.Copy(&employees4, user); len(*employees4) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	users := []User{User{Name: "Zhangsan", Age: 18, Role: "Admin", Notes: []string{"hello world"}}, User{Name: "Zhangsan2", Age: 22, Role: "Dev", Notes: []string{"hello world", "hello"}}}
	employees := []Employee{}

	if copit.Copy(&employees, users); len(employees) != 2 {
	...
	employees2 := &[]Employee{}
	if copit.Copy(&employees2, &users); len(*employees2) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copit.Copy(&employees3, users); len(employees3) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copit.Copy(&employees4, users); len(*employees4) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
...
func TestCopyFromSliceToSlice2(t *testing.T) {
	users := []*User{{Name: "Zhangsan", Age: 18, Role: "Admin", Notes: []string{"hello world"}}, nil}
	employees := []Employee{}

	if copit.Copy(&employees, users); len(employees) != 1 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees2 := &[]Employee{}
	if copit.Copy(&employees2, &users); len(*employees2) != 1 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if copit.Copy(&employees3, users); len(employees3) != 1 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if copit.Copy(&employees4, users); len(*employees4) != 1 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
